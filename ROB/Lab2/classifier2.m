function quality = classifier2(ts,test)
% 
% ts - training set
% test - test set
...
dens(:, 4) = mvnpdf(test(:,2:end),mu_spades,cov_spades);
dens(:, 3) = mvnpdf(test(:,2:end),mu_hearts,cov_hearts);
dens(:, 2) = mvnpdf(test(:,2:end),mu_diamonds,cov_diamonds);
dens(:, 1) = mvnpdf(test(:,2:end),mu_clubs,cov_clubs);
